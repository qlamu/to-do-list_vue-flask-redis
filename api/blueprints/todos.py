...
def todos(user_id: int, list_id: int):
    ...
    return {
        "status": 405,
        "message": "Requests to /lists/todos/<list_id> must be GET or PUT",
    }, 405
...
def crud_todos(user_id: int, list_id: int, todo_id: int):
    ...
    if not (
        redis_client.sismember(f"lists:{user_id}", list_id)
        and redis_client.sismember(f"list_content:{list_id}", todo_id)
    ):
        return {
            "status": 403,
    ...
    if request.method == "PATCH":
        ...
        if "description" in data:
            new_mapping["description"] = data["description"]
        if "is_done" in data:
    ...
    return {
        "status": 405,
        "message": "Requests to /lists/todos/<list_id>/<todo_id> must be GET, DELETE or PATCH",
    }, 405
